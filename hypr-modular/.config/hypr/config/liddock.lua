-- Lid / dock handling
--
-- Ported from the desktop's monolithic config. The problem it solves: closing
-- the laptop lid while docked leaves workspaces stranded on a screen you cannot
-- see. Hyprland has no built-in notion of "this output is now inside a closed
-- laptop", so this shuffles workspaces onto an external and puts them back on
-- open.
--
-- Wallpaper calls from the original are removed here: noctalia owns the
-- wallpaper on this machine and re-applies it across monitor changes itself.
--
-- Scenario matrix:
...
--
-- eDP-1 is kept enabled (not disabled) while the lid is closed: disabling it
...
-- Workspace kept on eDP-1 while the lid is closed so that 1-10 stay on the
-- externals. Deliberately outside the 1-10 range that workspaces.lua assigns.
local LID_PARK_WS    = "11"
